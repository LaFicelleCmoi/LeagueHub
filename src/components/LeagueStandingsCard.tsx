...
export function LeagueStandingsCard({ league, standings }: { league: League; standings: Standings | null }) {
  ...
  return (
    <SpotlightCard
      spotlightColor={withAlpha(league.accent, 0.2)}
      className="h-full rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"
    >
      ...
      <article style={{ "--accent": league.accent } as CSSProperties} className="relative flex h-full flex-col">
        ...
        <Link
          href={`/${league.slug}`}
          className="mt-auto flex items-center justify-between border-t border-slate-100 px-4 py-3 text-sm font-medium text-slate-600 hover:bg-slate-50/60 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800/50"
        >
          Classement complet
